...
export async function POST(req: NextRequest) {
  try {
    ...
    // TTFT optimization: billing tier lookup + agent resolution in parallel (~100ms saved)
    const [billingResult, agentResult] = await Promise.all([
      Promise.resolve(
        service
        ...
        .then(({ data: sub }) => (sub as { plan_tier?: string } | null)?.plan_tier ?? null)
        .catch((e: unknown) => {
    ...
    const billingTier = billingResult;
