...
Cron job business logic is extracted into `lib/cron-core/` as the **single source of truth**. Both Next.js route handlers and the OpenClaw scheduler import from this shared module. This eliminates Vercel Hobby plan's 10-second timeout bottleneck.

| Layer | Path | Role |
|-------|------|------|
| Core Logic | `lib/cron-core/*.ts` | Business logic (13 execute functions) |
| HTTP Wrappers | `app/api/cron/*/route.ts` | Thin wrappers: auth → execute → JSON response |
| Scheduler | `openclaw/src/scheduler.ts` | Direct function calls (no HTTP fetch) |
...
| **skills/proactive.md** | `lib/cron-core/heartbeat.ts` (20% chance when last chat > 2h) |
| **skills/artifact-creator.md** | `lib/artifacts/creator.ts` (poem, diary, unsent_letter, will). Invoked from heartbeat at 25% |
| **skills/dream-engine.md** | `lib/cron-core/dream.ts` (three-stage Groq: pattern → blend → reflect). dream_journal artifact + dream memory |
...
| **skills/social.md** | `lib/cron-core/social.ts` (3–5 turns, lexicon, dogma, secrets decision, lexicon learning) |
...
| `openclaw/src/scheduler.ts` | node-cron scheduler — 13 direct execution jobs + 1 HTTP (lifeline) |
