...
describe("shared memory CAS continuity", () => {
  it("prevents a stale process from overwriting newer owner memory", () => {
    ...
    expect(processA.save({
      expectedRevision: 0,
      consent: true,
      memories: [{ id: "m1", text: "likes quiet mornings" }],
    })).toEqual({ ok: true, revision: 1 });

    expect(processB.save({
      expectedRevision: 0,
      consent: true,
      memories: [{ id: "m2", text: "stale overwrite" }],
    })).toEqual({ ok: false, reason: "stale_revision", currentRevision: 1 });

    expect(processB.read()).toMatchObject({
      revision: 1,
  ...
  it("preserves owner isolation plus consent, revoke and delete boundaries", () => {
    ...
    expect(ownerA.save({ expectedRevision: 0, consent: false, memories: [{ id: "m1" }] }))
      .toEqual({ ok: false, reason: "memory_consent_required" });
    expect(ownerA.save({ expectedRevision: 0, consent: true, memories: [{ id: "m1", text: "private" }] }))
      .toEqual({ ok: true, revision: 1 });
