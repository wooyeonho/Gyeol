...
export const useChatStore = create<ChatStore>((set, get) => ({
  ...
  sendMessage: async (message: string, meta) => {
    ...
    await handleStreamResponse(message, set as (partial: ChatStore | Partial<ChatStore> | ((state: ChatStore) => ChatStore | Partial<ChatStore>), replace?: boolean) => void);
  },
  retryLastMessage: async () => {
    ...
    if (lastAsstMsg.role === "assistant" && lastAsstMsg.error && lastUserMsg.role === "user") {
      ...
      await handleStreamResponse(lastUserMsg.content, set as (partial: ChatStore | Partial<ChatStore> | ((state: ChatStore) => ChatStore | Partial<ChatStore>), replace?: boolean) => void);
    }
  }
}));
