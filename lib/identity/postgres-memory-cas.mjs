...
export function createPostgresMemoryCasAdapter({ query, ownerId }) {
  ...
  return {
    async read() {
      const result = await query("select revision, consent, revoked, deleted, memories, ai_identity from gyeol_memory_cas where owner_id=$1", [owner]);
      const row = result.rows[0];
      if (!row || row.deleted) return { ok: true, revision: 0, consent: false, revoked: false, memories: [], aiIdentity: AI_IDENTITY };
    ...
    async save({ expectedRevision, memories, consent }) {
      ...
      const result = expectedRevision === 0
        ? await query(`insert into gyeol_memory_cas(owner_id, revision, consent, revoked, deleted, memories, ai_identity)
            values($1,$2,true,false,false,$3::jsonb,'AI_COMPANION') on conflict do nothing returning revision`, [owner, next, JSON.stringify(memories)])
        : await query(`update gyeol_memory_cas set revision=$2, consent=true, revoked=false, deleted=false, memories=$3::jsonb, ai_identity='AI_COMPANION'
            where owner_id=$1 and revision=$4 and deleted=false returning revision`, [owner, next, JSON.stringify(memories), expectedRevision]);
    ...
    async revoke({ expectedRevision }) {
      const result = await query(`update gyeol_memory_cas set revision=$2, revoked=true, memories='[]'::jsonb, ai_identity='AI_COMPANION'
        where owner_id=$1 and revision=$3 and deleted=false returning revision`, [owner, expectedRevision + 1, expectedRevision]);
      if (result.rowCount !== 1) return { ok: false, reason: "stale_revision", currentRevision: (await this.read()).revision };
