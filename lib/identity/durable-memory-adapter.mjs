...
export class NonProductionFileMemoryRepository {
  ...
  async withOwnerLock(ownerId, operation) {
    ...
    try {
      await writeFile(lockPath, JSON.stringify({ pid: process.pid, acquiredAt: new Date().toISOString() }), { encoding: "utf8", flag: "wx", mode: 0o600 });
    } catch (error) {
      if (error?.code === "EEXIST") throw new Error("memory_store_locked");
      throw error;
    }
    try {
