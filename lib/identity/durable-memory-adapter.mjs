...
import { acquireOwnerFileLock, releaseOwnerFileLock } from "./owner-file-lock.mjs";
...
export class NonProductionFileMemoryRepository {
  constructor({ rootDir, lockStaleMs = 30_000, lockNow = () => Date.now() }) {
  ...
  lockPathFor(ownerId) {
    return join(this.rootDir, `${ownerKey(ownerId)}.lock`);
  }
  async withOwnerLock(ownerId, operation) {
    await mkdir(this.rootDir, { recursive: true });
    const lockPath = this.lockPathFor(ownerId);
    const lock = await acquireOwnerFileLock(lockPath, { staleMs: this.lockStaleMs, now: this.lockNow });
    try {
      return await operation();
    } finally {
      await releaseOwnerFileLock(lockPath, lock.token);
    }
  }
  ...
  async serialize(ownerId, operation) {
    ...
    const current = previous.then(
      () => this.withOwnerLock(ownerId, operation),
      () => this.withOwnerLock(ownerId, operation),
    );
