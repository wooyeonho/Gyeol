...
describe("checkSelfNaming", () => {
  ...
  it("does nothing if self_name already set", async () => {
    const updateFn = vi.fn();
    (createServiceClient as ReturnType<typeof vi.fn>).mockReturnValue({
      from: vi.fn().mockReturnValue({
        select: vi.fn().mockReturnValue({ eq: vi.fn().mockReturnValue({ single: vi.fn().mockResolvedValue({ data: { self_name: "Gyeol", total_messages: 30, config: {} } }) }) }),
        update: updateFn,
      }),
    });
