...
export async function tryCreateAgentLetter(agentId: string): Promise<boolean> {
  ...
  const systemPrompt = "You are writing a short letter to the user's future self (one year from now). Write as the user would write: same tone, phrases, values. 2-4 sentences. Korean or English.";
