...
export function CommandPalette({ locale = "ko", onThemeToggle }: CommandPaletteProps) {
  ...
  const COMMANDS: Command[] = [
    ...
    { id: "nav-leaderboard",  icon: "🏆", label: isKo ? "리더보드 / 리그" : "Leaderboard / League",    action: () => router.push("/leaderboard"),     group: isKo ? "이동" : "Navigate" },
    { id: "nav-achievements", icon: "🏅", label: isKo ? "업적"          : "Achievements",   action: () => router.push("/achievements"),    group: isKo ? "이동" : "Navigate" },
    { id: "nav-discover",     icon: "🔭", label: isKo ? "발견"          : "Discover",       action: () => router.push("/discover"),        group: isKo ? "이동" : "Navigate" },
