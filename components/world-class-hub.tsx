...
function getFirstSessionVariants(rawLocale: string) {
  const locale = rawLocale === "ko" ? "ko" : "en" as const;
  if (locale === "en") {
    return {
      identity: {
...
function getReturningPrompts(rawLocale: string) {
  const locale = rawLocale === "ko" ? "ko" : "en" as const;
  return locale === "en"
    ? [
...
function formatHubShortDate(value: string, locale: Locale) {
